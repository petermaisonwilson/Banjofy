...
APP_TITLE = "Banjofy Song Analysis Laboratory 028 — Musical Tempo Selector"
...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            outer,
            text=(
                "This build strengthens whole-song interpretation by selecting the musical tempo level from half-time, detected-time and double-time candidates."
            ),
            wraplength=950,
        ...
        self.run_button = ttk.Button(
            controls_top, text="Detect 3/4 or 4/4 and Create Audible Check",
            command=self._start, state="disabled",
        )
        self.run_button.pack(side="left")
        self.play_button = ttk.Button(
            controls_top,
            text="Play Chord + Downbeat Check",
        ...
        self.recommend_button = ttk.Button(
            controls_bottom,
            text="Recommend Meter, Grid and Phase",
        ...
        self.phase_button = ttk.Button(
            controls_bottom,
            text="Test Phase Challenger",
            command=self._test_phase_challenger,
            state="disabled",
        )
        self.phase_button.pack(side="left", padx=8)
        ttk.Button(
            controls_bottom,
        ...
        benchmark_row.pack(fill="x", pady=(7, 0))
        self.benchmark_button = ttk.Button(
            benchmark_row,
            text="Benchmark All Verified Songs",
            command=self._benchmark_all_verified_songs,
        )
        ...
        self.interpreter_button = ttk.Button(
            interpreter_row,
            text="Run Musical Tempo Selector",
    ...
    def _selected(self, _event=None) -> None:
        ...
        self.phase_button.configure(state="normal")
    ...
    def _poll(self) -> None:
        while True:
            ...
            if kind == "status":
                ...
                self.status_var.set("Library timing benchmark complete")
                self._append("")
                self._append(f"Benchmark report: {payload['text_path']}")
                self._append(f"Benchmark JSON: {payload['json_path']}")
                messagebox.showinfo(
                    APP_TITLE,
                    (
                        "Library timing benchmark complete.\n\n"
                        f"Verified songs tested: {payload['song_count']}\n"
                        f"Best approach: {payload['best_engine']}\n"
                        f"Exact timing matches: {payload['best_exact']}\n\n"
                        "Open timing_engine_benchmark_026.txt in the Library root."
                    ),
                )
            elif kind == "benchmark_error":
                self.benchmark_button.configure(state="normal")
                self.status_var.set("Library timing benchmark failed")
                self._append(str(payload))
                messagebox.showerror(
                    APP_TITLE,
                    "The Library timing benchmark failed. The exact error is shown in the window.",
                )
            elif kind == "phase_done":
                self.phase_button.configure(state="normal")
                self.status_var.set("Phase challenger test complete")
                self._append("")
                self._append(f"Phase report: {payload['text_path']}")
                self._append(f"Old phase: {payload['old_phase']} — {payload['old_result']}")
                self._append(f"Challenger phase: {payload['new_phase']} — {payload['new_result']}")
                messagebox.showinfo(
                    APP_TITLE,
                    (
                        "Phase challenger test complete.\n\n"
                        f"Frozen meter: {payload['meter']}\n"
                        f"Frozen beat grid: {payload['grid']}\n"
                        f"Verified phase: {payload['truth_phase']}\n\n"
                        f"Old phase: {payload['old_phase']} — {payload['old_result']}\n"
                        f"Challenger phase: {payload['new_phase']} — {payload['new_result']}\n\n"
                        "Open phase_challenger_025.txt in the Analysis folder."
                    ),
                )
            elif kind == "phase_error":
                self.phase_button.configure(state="normal")
                self.status_var.set("Phase challenger test failed")
                self._append(str(payload))
                messagebox.showerror(
                    APP_TITLE,
                    "Phase challenger test failed. The exact error is shown in the window.",
                )
                ...
                messagebox.showinfo(
                    APP_TITLE,
                    (
                        "Build 028 recovered and consolidated the saved manual timing data.\n\n"
                ...
                messagebox.showinfo(
                    APP_TITLE,
                    (
                        "Build 028 created the complete timing evidence report.\n\n"
    ...
    def _recommend_timing(self) -> None:
        ...
        self.recommend_button.configure(state="disabled")
        self.status_var.set("Build 028 is scoring meter, beat grid and downbeat phase…")
    ...
    def _run_whole_song_interpreter(self) -> None:
        ...
        self.status_var.set("Build 028 is interpreting every verified song…")
        self._append("")
        self._append(
    ...
    def _benchmark_all_verified_songs(self) -> None:
        ...
        self.status_var.set("Build 028 is benchmarking all verified songs…")
        self._append("")
        self._append(f"Started Library benchmark for {len(verified)} verified songs.")

        def progress(text: str) -> None:
            self.messages.put(("benchmark_progress", text))

        def worker() -> None:
            try:
                result = structure_engine.benchmark_library_timing_engines(
                    verified,
                    progress_callback=progress,
                )
                json_path = library_root / "timing_engine_benchmark_026.json"
                text_path = library_root / "timing_engine_benchmark_026.txt"
                write_json_atomic(json_path, result)
                text_path.write_text(
                    structure_engine.format_timing_engine_benchmark(result),
                    encoding="utf-8",
                )
                leader = result["leaderboard"][0]
                self.messages.put(("benchmark_done", {
                    "json_path": str(json_path),
                    "text_path": str(text_path),
                    "song_count": result["verified_song_count"],
                    "best_engine": leader["label"],
                    "best_exact": f"{leader['exact_matches']}/{result['verified_song_count']}",
                }))
            except Exception as exc:
                self.messages.put(("benchmark_error", f"{exc}\\n\\n{traceback.format_exc()}"))

        threading.Thread(target=worker, daemon=True).start()

    def _test_phase_challenger(self) -> None:
        if self.selected_song is None:
            messagebox.showinfo(APP_TITLE, "Select an analysed song first.")
            return
        ...
        song = dict(self.selected_song)
        self.phase_button.configure(state="disabled")
        self.status_var.set("Build 028 is testing the phase challenger…")
        ...
        def worker() -> None:
            try:
                ...
                result = structure_engine.test_phase_challenger(
                ...
                json_path = folder / "phase_challenger_025.json"
                text_path = folder / "phase_challenger_025.txt"
                write_json_atomic(json_path, result)
                text_path.write_text(
                    structure_engine.format_phase_challenger(result),
                    encoding="utf-8",
                )

                self.messages.put(("phase_done", {
                    "text_path": str(text_path),
                    "json_path": str(json_path),
                    "meter": result["frozen_meter"],
                    "grid": result["frozen_beat_grid_method"],
                    "truth_phase": result["verified_phase_number"],
                    "old_phase": result["old_phase_number"],
                    "new_phase": result["challenger_phase_number"],
                    "old_result": result["old_phase_result"],
                    "new_result": result["challenger_phase_result"],
                }))
            except Exception as exc:
                self.messages.put(("phase_error", f"{exc}\\n\\n{traceback.format_exc()}"))
    ...
    def _validate_automatic_timing(self) -> None:
        ...
        song = dict(self.selected_song)
        self.validate_button.configure(state="disabled")
        self.status_var.set("Build 028 is comparing automatic timing with verified truth…")
    ...
    def _recover_manual_truth(self) -> None:
        ...
        song = dict(self.selected_song)
        self.truth_button.configure(state="disabled")
        self.status_var.set("Build 028 is locating explicit confirmations…")
    ...
    def _show_truth_review(self, payload: dict) -> None:
        ...
        dialog.title("Build 028 — Review Verified Timing Truth")
    ...
    def _create_timing_evidence_report(self) -> None:
        ...
        self.evidence_button.configure(state="disabled")
        self.status_var.set("Build 028 is calculating the complete candidate evidence…")
    ...
    def _play_alternative_beat_grids(self) -> None:
        ...
        candidates = analysis.get("alternative_beat_grids")
        if not isinstance(candidates, dict) or not candidates:
            messagebox.showerror(APP_TITLE, "Create the Build 028 alternative beat grids first.")
            return
        valid = {
        ...
        if not valid:
            messagebox.showerror(APP_TITLE, "The Build 028 alternative beat files are missing.")
            return
    ...
    def _confirm_selected_alternative_grid(self) -> None:
        ...
        if not messagebox.askyesno(
            ...
            (
                ...
                f"Build 028 will retain the original detector grid, rebuild {meter} bars "
        ...
        try:
            ...
            messagebox.showinfo(
                ...
                (
                    ...
                    f"Build 028 created {len(phase_paths)} fresh {meter} phase auditions. "
    ...
    def _play_audible_check(self) -> None:
        ...
        if len(valid_paths) != expected_count:
            self.status_var.set(
                "Rebuilding missing or mismatched phase audition files in Build 028…"
            )
            ...
            self._append(
                f"Build 028 rebuilt {len(valid_paths)} phase audition files."
            )
        ...
        if len(self.visual_phase_paths) != self.visual_beats_per_bar:
            messagebox.showerror(
                APP_TITLE,
                "Build 028 could not create the required phase audition files. "
