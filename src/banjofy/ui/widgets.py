...
class BanjoDiagram(QWidget):
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        for i, label in enumerate(labels):
            ...
            painter.drawText(int(x - 5), int(top + grid_h + 22), label)
        ...
        for i, fret in enumerate(self._shape.frets):
            ...
            if fret == 0:
                ...
                painter.drawEllipse(int(x - 5), 8, 10, 10)
            else:
                ...
                if finger:
                    painter.setPen(QPen(dot, 1))
                    painter.drawText(int(x - 5), int(y + 5), str(finger))
...
class ChordPanel(QFrame):
    def __init__(self, title: str, chord: str, accent: str, subtitle: str = "", parent=None) -> None:
        ...
        header.setStyleSheet(f"color: {accent}; font-size: 14px;")
        layout.addWidget(header)

        row = QHBoxLayout()
        self.letter = QLabel(chord)
        self.letter.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.letter.setStyleSheet("font-size: 58px; font-weight: bold; color: #f4e2bd;")
