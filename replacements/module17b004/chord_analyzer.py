...
class ChordAnalyzer:
    ...
    def _templates(self):
        ...
        return out
    def _classify(self,c,tonic,mode,candidates=None):
        ...
        for name,r,q,t in self._templates():
            if candidates and name not in set(candidates):continue
    ...
    def analyse(self,audio_path,beat_times_ms,beats_per_bar,progress=None,candidate_chords=None,supplied_tonal_center=None):
        ...
        x=self._decode(audio_path); auto,mode,keyconf=self._tonic(self._global(x)); ov=self.tonal_center_override(audio_path); supplied=supplied_tonal_center if supplied_tonal_center in self.NOTE_NAMES else None; chosen=ov or supplied; tonic=self.NOTE_NAMES.index(chosen) if chosen else auto; tonal=chosen or self.NOTE_NAMES[auto]; source='Manual' if ov else ('Online consensus' if supplied else 'Automatic')
