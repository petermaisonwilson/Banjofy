...
from banjofy.ui.chord_analyzer import ChordAnalysis, ChordAnalyzer
from banjofy.ui.online_chord_resolver import ChordDataConsensus, OnlineChordResolver


APP_VERSION = "Banjofy 006.3.0 Module 17 Build 004 - Online Chord Consensus"


class MainWindow(LegacyMainWindow):
    """Clean Module 17 Build 002 window built on the confirmed Module 16 foundation.
    ...
    def __init__(self) -> None:
        ...
        self.chord_analysis: ChordAnalysis | None = None
        self.chord_queue: queue.Queue = queue.Queue()
        self.chord_analysis_running = False

        self.online_chord_resolver = OnlineChordResolver()
        self.chord_data_consensus: ChordDataConsensus | None = None
        self.online_queue: queue.Queue = queue.Queue()
        self.online_lookup_running = False
        ...
        self.online_poll_timer = QTimer(self)
        self.online_poll_timer.setInterval(100)
        self.online_poll_timer.timeout.connect(self._poll_online_chord_lookup)

        self.statusBar().showMessage("Ready - Module 17 Build 004 online chord consensus loaded")

    def _set_visible_titles(self) -> None:
        for label in self.findChildren(QLabel):
            if label.objectName() == "Title":
                if "Practice" in label.text():
                    label.setText("Practice Studio - Module 17 Build 004")
                else:
    ...
    def _build_ui(self) -> None:
        ...
        for label in hints:
            if label.objectName() == "Hint":
                label.setText(
                    "Module 17 clean build: real neutral chord analysis, 3/4 or 4/4 meter "
                    "and corrected grid positioning when songs change."
                )
                break
    ...
    def _build_practice_page(self) -> QWidget:
        ...
        meter_row.addWidget(self.meter_button)

        self.tonal_center_button = QPushButton("Tonal centre: Auto")
        self.tonal_center_button.setToolTip("Cycle Auto and C through B")
        self.tonal_center_button.clicked.connect(self._cycle_tonal_center_override)
        meter_row.addWidget(self.tonal_center_button)
        outer.addLayout(meter_row)

        self.online_chord_status_label = QLabel("Online chord data: load a song first")
        self.online_chord_status_label.setObjectName("LibraryMessage")
        self.online_chord_status_label.setWordWrap(True)
        outer.addWidget(self.online_chord_status_label)

        self.detected_song_chords_label = QLabel("Candidate song chords: —")
        self.detected_song_chords_label.setObjectName("LibraryMessage")
        self.detected_song_chords_label.setWordWrap(True)
        outer.addWidget(self.detected_song_chords_label)

        self.chord_status_label = QLabel("Chords: load a song first")
    ...
    def _display_chord_for_beat(self, beat_index: int) -> str:
        analysis = self.chord_analysis
        if analysis is None or beat_index >= len(analysis.beat_chords):
            return "•"

        chord = analysis.beat_chords[beat_index]
        if chord in ("", "N"):
            return "•"

        if beat_index > 0:
            previous = analysis.beat_chords[beat_index - 1]
            if previous == chord:
                return "•"
    ...
    def _schedule_grid_reset(self) -> None:
        # Run after Qt has completed the new grid layout and restored scroll ranges.
    ...
    def _load_selected_song_into_practice(self) -> None:
        ...
        self.chord_data_consensus = None
        self.online_chord_status_label.setText("Online chord data: waiting for timing analysis")
        self.detected_song_chords_label.setText("Candidate song chords: —")
        self.chord_status_label.setText("Chords: waiting for timing analysis")
        self.meter_button.setText("Meter: Auto")
        override = self.chord_analyzer.tonal_center_override(Path(self.practice_song.audio_file))
        self.tonal_center_button.setText(f"Tonal centre: {override}" if override else "Tonal centre: Auto")
    ...
    def _build_beat_grid(self, song: LibrarySong) -> None:
        ...
        if self.chord_analysis is not None:
            for beat_index, cell in enumerate(self.grid_cells):
                if beat_index < len(self.chord_analysis.beat_chords):
                    chord_name = self.chord_analysis.beat_chords[beat_index]
                    cell.setText(self._display_chord_for_beat(beat_index))
    ...
    def _build_meter_aware_grid(self, song: LibrarySong, beats_per_bar: int) -> None:
        ...
        for bar in range(bars):
            ...
            for beat in range(beats_per_bar):
                beat_index = bar * beats_per_bar + beat
                if (
                    self.chord_analysis is not None
                    and beat_index < len(self.chord_analysis.beat_chords)
                ):
                    chord_name = self.chord_analysis.beat_chords[beat_index]
                    cell_text = self._display_chord_for_beat(beat_index)
                else:
    ...
    def _highlight_beat(self, beat_index: int) -> None:
        ...
        if self.chord_analysis is not None and self.grid_cells:
            beat_index = max(0, min(beat_index, len(self.grid_cells) - 1))
            for i, cell in enumerate(self.grid_cells):
                if i >= len(self.chord_analysis.beat_chords):
                    continue
                chord_name = self.chord_analysis.beat_chords[i]
                base_text = self._display_chord_for_beat(i)
                if i == beat_index:
    ...
    def _library_song_record_path(self, song: LibrarySong) -> Path:
        ...
        # Analysis and song folders are siblings in the configured Library.
        library_root = analysis_path.parent.parent if analysis_path.parent.name else analysis_path.parent
        possible = library_root / "songs" / f"{safe}.song.json"
        if possible.exists():
            return possible

        # Fallback: locate the exact matching record among loaded song files.
        for parent in (analysis_path.parent, analysis_path.parent.parent):
            songs_dir = parent / "songs"
            if songs_dir.exists():
                candidate = songs_dir / f"{safe}.song.json"
                if candidate.exists():
                    return candidate
        return possible
    ...
    def _delete_selected_library_song(self) -> None:
        ...
        answer = QMessageBox.question(
            ...
            "The Library record, saved analysis, timing cache and chord cache "
            "will be deleted. Audio is handled separately.",
        ...
        # Stop and release the Windows media file handle before deleting anything.
        try:
            self._practice_stop()
            self.media_player.stop()
        ...
        try:
            self.chord_analyzer.delete_cache(audio_path)
        except Exception as exc:
            errors.append(f"chord cache: {exc}")
    ...
    def _poll_timing_analysis(self) -> None:
        while True:
            ...
            self._schedule_grid_reset()
            self._start_online_chord_lookup()
            return
    ...
    def _cycle_meter_override(self) -> None:
        if not self.practice_song or self.timing_analysis is None:
            self.practice_message.setText("Load a song and complete timing analysis first.")
            return
        ...
        if chosen is None:
            ...
            self.practice_message.setText("Meter returned to automatic detection.")
            self._start_automatic_timing_analysis()
            return
        ...
        self.practice_message.setText(f"Meter corrected to {chosen}/4 and saved.")

    # ---------- Tonal centre correction ----------

    def _cycle_tonal_center_override(self) -> None:
        if not self.practice_song:
            self.practice_message.setText("Load a song into Practice first.")
            return
        choices = ["Auto"] + list(self.chord_analyzer.NOTE_NAMES)
        current = self.tonal_center_button.text().replace("Tonal centre: ", "")
        try:
        ...
        audio_path = Path(self.practice_song.audio_file)
        self.chord_analyzer.save_tonal_center_override(
            audio_path, None if selected == "Auto" else selected
        )
        self.tonal_center_button.setText(f"Tonal centre: {selected}")
        self.chord_analysis = None
        self._start_chord_analysis()
        self.practice_message.setText(
            "Tonal centre returned to Auto." if selected == "Auto"
            else f"Tonal centre corrected to {selected} and saved."
        )

    # ---------- Online chord-data consensus ----------

    def _start_online_chord_lookup(self) -> None:
        if not self.practice_song:
            return
        self.online_lookup_running = True
        self.online_chord_status_label.setText(
            "Online chord data: searching multiple sources..."
        )
        artist = self.practice_song.channel
        title = self.practice_song.title

        def worker() -> None:
            try:
                result = self.online_chord_resolver.resolve(
                    artist,
                    title,
                    progress=lambda message: self.online_queue.put(("progress", message)),
                )
                self.online_queue.put(("done", result))
            except Exception as exc:
                self.online_queue.put(("error", str(exc)))

        threading.Thread(target=worker, daemon=True).start()
        self.online_poll_timer.start()

    def _poll_online_chord_lookup(self) -> None:
        while True:
            try:
                kind, payload = self.online_queue.get_nowait()
            except queue.Empty:
                return

            if kind == "progress":
                self.online_chord_status_label.setText(
                    f"Online chord data: {payload}"
                )
                continue

            self.online_poll_timer.stop()
            self.online_lookup_running = False

            if kind == "error":
                self.chord_data_consensus = None
                self.online_chord_status_label.setText(
                    f"Online chord data unavailable: {payload} | "
                    "Using audio-only fallback."
                )
                self.detected_song_chords_label.setText(
                    "Candidate song chords: audio-only unrestricted"
                )
                self._start_chord_analysis()
                return

            self.chord_data_consensus = payload
            sources = []
            for finding in payload.findings:
                if finding.source_name not in sources:
                    sources.append(finding.source_name)
            chord_text = " | ".join(payload.chords) if payload.chords else "none"
            self.online_chord_status_label.setText(
                f"Online chord consensus: key {payload.key or 'not agreed'} | "
                f"{len(payload.findings)} findings | "
                f"sources: {', '.join(sources[:5])}"
            )
            self.detected_song_chords_label.setText(
                f"Candidate song chords: {chord_text}"
            )
            self._start_chord_analysis()
            return

    # ---------- Neutral chord analysis ----------

    def _start_chord_analysis(self) -> None:
        if (
            not self.practice_song
            or self.timing_analysis is None
            or not self.timing_analysis.usable
        ):
            return

        audio_path = Path(self.practice_song.audio_file)
        self.chord_analysis_running = True
        self.chord_status_label.setText("Chords: analysing detected beats...")

        def worker() -> None:
            try:
                consensus = self.chord_data_consensus
                candidate_chords = consensus.chords if consensus and consensus.usable else None
                supplied_key = consensus.key if consensus else None
                if supplied_key and supplied_key.endswith("m"):
                    supplied_key = supplied_key[:-1]
                result = self.chord_analyzer.analyse(
                    audio_path,
                    self.timing_analysis.beat_times_ms,
                    self._beats_per_bar(),
                    progress=lambda message: self.chord_queue.put(("progress", message)),
                    candidate_chords=candidate_chords,
                    supplied_tonal_center=supplied_key,
                )
    ...
    def _poll_chord_analysis(self) -> None:
        while True:
            ...
            if kind == "progress":
                self.chord_status_label.setText(f"Chords: {payload}")
                continue
            ...
            if kind == "error":
                ...
                self.chord_status_label.setText(
                    f"Chords: analysis failed safely | Exact failure: {payload}"
                )
                return

            self.chord_analysis = payload
            if self.practice_song is not None:
                key_text = (
                    payload.tonal_center
                    if payload.mode == "Power/ambiguous"
                    else f"{payload.tonal_center} {payload.mode}"
                )
                self.practice_song = replace(self.practice_song, key=key_text)
            confidence_percent = int(round(payload.confidence * 100))
            placed_unique = []
            for chord_name in payload.beat_chords:
                if chord_name not in ("", "N") and chord_name not in placed_unique:
                    placed_unique.append(chord_name)
            placed_text = " | ".join(placed_unique) if placed_unique else "none"
            if self.chord_data_consensus is not None:
                candidate_text = " | ".join(self.chord_data_consensus.chords)
                self.detected_song_chords_label.setText(
                    f"Candidate song chords: {candidate_text} | "
                    f"Placed in grid: {placed_text}"
                )
            else:
                self.detected_song_chords_label.setText(
                    f"Audio-detected song chords: {placed_text}"
                )

            self.chord_status_label.setText(
                f"Chords source: {payload.source_kind} | "
                f"Tonal centre: {payload.tonal_center} ({payload.tonal_center_source}) | "
                f"Mode: {payload.mode} | "
                f"confidence {confidence_percent}% | {payload.diagnostic}"
            )
            self._build_beat_grid(self.practice_song)
            ...
            self._schedule_grid_reset()
            self.practice_message.setText(
                "Real neutral chord analysis ready. Grid labels now come from the audio."
            )
            return
