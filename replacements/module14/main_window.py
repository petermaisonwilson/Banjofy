...
APP_VERSION = "Banjofy 006.3.0 Module 14 Build 001 - Single and Continuous Repeat"


class MainWindow(LegacyMainWindow):
    """Clean Module 14 window built on the confirmed Module 13 foundation.
    ...
    def __init__(self) -> None:
        ...
        self.statusBar().showMessage("Ready - Module 14 single and continuous repeat loaded")

    def _set_visible_titles(self) -> None:
        for label in self.findChildren(QLabel):
            if label.objectName() == "Title":
                if "Practice" in label.text():
                    label.setText("Practice Studio - Module 14")
                else:
    ...
    def _build_ui(self) -> None:
        ...
        for label in hints:
            if label.objectName() == "Hint":
                label.setText(
                    "Module 14 clean build: single-run or continuous repeat with a fresh "
                    "one-second count-in before every practice run."
                )
                break
    ...
    def _build_practice_page(self) -> QWidget:
        ...
        for label in labels:
            if label.objectName() == "Title":
                ...
                label.setText(
                    "Choose 2-5 one-second counts. Select Single Run or Continuous Practice "
                    "for the chosen S-to-E section."
                )
