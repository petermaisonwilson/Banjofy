...
APP_VERSION = "Banjofy 006.3.0 Module 15 Build 001 - Automatic Musical Timing"


class MainWindow(LegacyMainWindow):
    """Clean Module 15 window built on the confirmed Module 14 foundation.
    ...
    def __init__(self) -> None:
        ...
        self.statusBar().showMessage("Ready - Module 15 automatic musical timing loaded")

    def _set_visible_titles(self) -> None:
        for label in self.findChildren(QLabel):
            if label.objectName() == "Title":
                if "Practice" in label.text():
                    label.setText("Practice Studio - Module 15")
                else:
    ...
    def _build_ui(self) -> None:
        ...
        for label in hints:
            if label.objectName() == "Hint":
                label.setText(
                    "Module 15 clean build: automatic beat timestamps and downbeat alignment "
                    "for accurate seeking and repeat practice."
                )
                break
    ...
    def _build_practice_page(self) -> QWidget:
        ...
        for label in labels:
            if label.objectName() == "Title":
                ...
                label.setText(
                    "Timing is analysed automatically. Grid seeking and repeats use detected "
                    "musical beats rather than assuming the song starts at 00:00."
                )
    ...
    def _build_beat_grid(self, song: LibrarySong) -> None:
        ...
        if timing is not None and timing.usable:
            detected_bars = max(1, (len(timing.beat_times_ms) + 3) // 4)
    ...
    def _poll_timing_analysis(self) -> None:
        while True:
            ...
            if kind == "error":
                self.timing_analysis = None
                self.timing_status_label.setText(
                    f"Timing: automatic analysis unavailable — using BPM fallback. {payload}"
                )
                self.practice_message.setText(
            ...
            bars = max(1, (len(payload.beat_times_ms) + 3) // 4)
            confidence_percent = int(round(payload.confidence * 100))
            self.timing_status_label.setText(
                f"Timing: {len(payload.beat_times_ms)} beats / {bars} bars detected | "
                f"first downbeat {payload.first_downbeat_ms / 1000:.2f}s | "
                f"downbeat confidence {confidence_percent}%"
            )
            self.practice_message.setText(
