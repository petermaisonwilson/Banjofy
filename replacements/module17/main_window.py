...
APP_VERSION = "Banjofy 006.3.0 Module 17 Build 001 - Real Chord Analysis"


class MainWindow(LegacyMainWindow):
    """Clean Module 17 window built on the confirmed Module 16 foundation.
    ...
    def _force_grid_to_start(self) -> None:
        if hasattr(self, "grid_scroll"):
            self.grid_scroll.verticalScrollBar().setValue(
                self.grid_scroll.verticalScrollBar().minimum()
            )
            self.grid_scroll.horizontalScrollBar().setValue(
                self.grid_scroll.horizontalScrollBar().minimum()
            )
        if self.grid_cells:
        ...
        if self.chord_analysis is not None:
            for beat_index, cell in enumerate(self.grid_cells):
                if beat_index < len(self.chord_analysis.beat_chords):
                    ...
                    cell.setText(chord_name if chord_name != "N" else "•")
        ...
        for bar in range(bars):
            ...
            for beat in range(beats_per_bar):
                ...
                if (
                    ...
                    chord_name = self.chord_analysis.beat_chords[beat_index]
                    cell_text = chord_name if chord_name != "N" else "•"
                else:
    ...
    def _highlight_beat(self, beat_index: int) -> None:
        ...
        if self.chord_analysis is not None and self.grid_cells:
            ...
            for i, cell in enumerate(self.grid_cells):
                ...
                base_text = chord_name if chord_name != "N" else "•"
                if i == beat_index:
    ...
    def _delete_selected_library_song(self) -> None:
        ...
        answer = QMessageBox.question(
            ...
            "The saved analysis and timing cache will be deleted. "
            "The downloaded audio will be kept unless you choose to remove it next.",
        ...
        if self.practice_song and self.practice_song.audio_file == song.audio_file:
            ...
            self.practice_message.setText(
                "The deleted Library song was removed from Practice."
            )
        ...
        if errors:
            self._set_library_message(
                "Library entry removed, but some files could not be deleted: "
                + "; ".join(errors)
            )
        else:
            self._set_library_message(f"Deleted from Library: {song.title}")
    ...
    def _poll_chord_analysis(self) -> None:
        while True:
            ...
            self.chord_status_label.setText(
                ...
                f"{len(payload.beat_chords)} beat labels | "
                f"confidence {confidence_percent}% | {payload.diagnostic}"
            )
            self._build_beat_grid(self.practice_song)
            self._update_grid_cursor_from_position(self.media_player.position())
